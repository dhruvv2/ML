...
class EightPuzzleState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        original_zero_loc = self.zero_loc
        
        if original_zero_loc[0] < 2:
            below = copy.deepcopy(self.state)
            below[original_zero_loc[0]][original_zero_loc[1]] = below[original_zero_loc[0]+1][original_zero_loc[1]]
            below[original_zero_loc[0]+1][original_zero_loc[1]] = 0
            below_state = EightPuzzleState(below, self.goal, self.dist_from_start+1, self.use_heuristic, (original_zero_loc[0] + 1, original_zero_loc[1]))
            nbr_states.append(below_state)

        if original_zero_loc[1] > 0:
            left = copy.deepcopy(self.state)
            left[original_zero_loc[0]][original_zero_loc[1]] = left[original_zero_loc[0]][original_zero_loc[1]-1]
            left[original_zero_loc[0]][original_zero_loc[1]-1] = 0
            left_state = EightPuzzleState(left, self.goal, self.dist_from_start+1, self.use_heuristic, (original_zero_loc[0], original_zero_loc[1] - 1))
            nbr_states.append(left_state)

        if original_zero_loc[0] > 0:
            above = copy.deepcopy(self.state)
            above[original_zero_loc[0]][original_zero_loc[1]] = above[original_zero_loc[0]-1][original_zero_loc[1]]
            above[original_zero_loc[0]-1][original_zero_loc[1]] = 0
            above_state = EightPuzzleState(above, self.goal, self.dist_from_start+1, self.use_heuristic, (original_zero_loc[0] - 1, original_zero_loc[1]))
            nbr_states.append(above_state)
        
        if original_zero_loc[1] < 2:
            right = copy.deepcopy(self.state)
            right[original_zero_loc[0]][original_zero_loc[1]] = right[original_zero_loc[0]][original_zero_loc[1]+1]
            right[original_zero_loc[0]][original_zero_loc[1]+1] = 0
            right_state = EightPuzzleState(right, self.goal, self.dist_from_start+1, self.use_heuristic, (original_zero_loc[0], original_zero_loc[1] + 1))
    ...
    def compute_heuristic(self):
        ...
        for i in range(3):
            for j in range(3):
                for k in range(3):
                    for l in range(3):
                        if self.state[i][j] != 0 and self.state[i][j] == self.goal[k][l]:
                            total += manhattan((i,j), (k,l))
...
class SingleGoalGridState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        for coords in (neighboring_grid_locs):
            nbr_states.append(SingleGoalGridState(coords, self.goal, self.dist_from_start+1, self.use_heuristic, self.maze_neighbors))
...
class GridState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        for coords in (neighboring_locs):
            new_state = GridState(coords, self.goal, self.dist_from_start+1, self.use_heuristic, self.maze_neighbors, self.mst_cache)
            if coords in self.goal:
                for goal in new_state.goal:
                    if coords == goal:
                        goal_list = list(new_state.goal)
                        goal_list.remove(goal)
                        new_state.goal = tuple(goal_list)
    ...
    def compute_heuristic(self):
        if len(self.goal) == 1:
            ...
            min = manhattan(self.state, self.goal[0])
            min_goal = (0,0)
            print(self.state)
            for goal in self.goal:
                curr_dist = manhattan(self.state, goal)
                if min > curr_dist:
                    min = curr_dist
                    min_goal = goal
            print(self.goal)
            ...
            if self.goal in self.mst_cache:
                mst_val = self.mst_cache[self.goal]
            else:
                mst_val = compute_mst_cost(self.goal, manhattan)
                self.mst_cache[self.goal] = mst_val
