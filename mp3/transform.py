...
def transformToMaze(alien, goals, walls, window,granularity):
    ...
    """
    # print('hello')
    # print(alien.get_alien_limits())

    rows = (alien.get_alien_limits()[1][1] // granularity) + 1
    cols = (alien.get_alien_limits()[0][1] // granularity) + 1
    shapes = alien.get_alien_limits()[2][1]

    maze_arr = np.empty((cols, rows, shapes), dtype='str')
    maze_arr[:][:][:] = ' '

    config = alien.get_config()
    start = configToIdx(config, [0,0,0], granularity, alien)
    maze_arr[start[0]][start[1]][start[2]] = 'P'

    for i in range(cols):
        for j in range(rows):
            for k in range(shapes):
                new_config = idxToConfig((i,j,k), [0,0,0], granularity, alien)
                alien.set_alien_config(new_config)
    ...
    return Maze(maze_arr, alien, {}, granularity, [0,0,0], None, True)
